...
describe('telling one kind of source from another by its address', () => {
  ...
  it('sites come first, then Loop, then OneDrive, then the group inboxes, the To Do lists and the teams, and the order inside each is left alone', () => {
    const listed = [
    ...
    expect(orderByKind(listed).map(kindOf)).toEqual(['site', 'site', 'loop', 'loop', 'onedrive', 'group', 'todo', 'team']);
