...
export type AddressKind = 'site' | 'loop' | 'onedrive';
...
const RANK: Readonly<Record<AddressKind, number>> = { site: 0, loop: 1, onedrive: 2 };
...
export const orderByKind = <T extends { readonly webUrl: string }>(sources: ReadonlyArray<T>): ReadonlyArray<T> =>
  [...sources].sort((left, right) => RANK[addressKindOf(left.webUrl)] - RANK[addressKindOf(right.webUrl)]);
